...
export default function NavBar() {
    ...
    return (
        <nav
            className={`sticky top-0 left-0 m-4 mr-0 flex lg:flex-col lg:h-[96vh] lg:p-6 p-3 flex-row max-lg:w-[calc(100%-32px)] max-lg:ml-4 max-lg:items-center justify-between rounded-lg bg-gray-100 shadow-md transition-all duration-300 dark:bg-gray-800 ${isExpanded ? 'w-64' : 'w-20'}`}
            onMouseEnter={() => setIsExpanded(true)}
            onMouseLeave={() => setIsExpanded(false)}
        >
            {/* Header */}
            <div className="max-lg:hidden flex flex-col items-center gap-3">
            ...
            {/* Links */}
            <div className={`flex lg:flex-col lg:mt-4 lg:gap-2 flex-row max-lg:items-center gap-4 font-medium text-gray-700 dark:text-gray-200`}>
                {(
                    [
                        'dashboard',
                        'incomes',
                    ...
                    >
                        <i
                            className={`bxr bx-${key === 'dashboard'
                                ? 'pie-chart-alt'
                                : key === 'incomes'
                                    ? 'wallet-note'
                                    : key === 'expenses'
                        ...
                        ></i>
                        <span className={`${textAnimation("text-nowrap max-lg:hidden")}`}>{t(key)}</span>
                    </NavLink>
                ))}
            </div>
